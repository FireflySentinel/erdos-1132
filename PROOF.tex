...
These conclusions answer both questions in Erd\H{o}s Problem 1132.
The first proof combines Tao's local potential estimates with a local
Riesz differentiation formula, an energy estimate for nodal derivative
jumps, and a second moment argument. The second proof uses positive
Cauchy transforms and harmonic measure.
\end{abstract}
...
constant at a fixed point. Theorem~\ref{thm:main}(ii) establishes the
sharp normalized lower bound on a set of full measure.

For the first assertion, a Baire category reduction gives an interval
on which \(\lambda_n\le(2/\pi)\log n+O(1)\), if the assertion is
false. On that interval, Tao's potential estimates and a local Riesz
formula control derivatives of all interpolants with bounded nodal
data. A positive definite kernel then gives a sharp lower average for
the derivative jumps of \(\lambda_n\) at the nodes. This produces
linearly many separated points with values at least
\((2/\pi)\log n-O(1)\). A second moment argument yields a set of
positive measure on which this lower bound recurs infinitely often.

The proof of the almost-everywhere assertion is separate. Normalized
barycentric weights define two positive Cauchy transforms whose
difference is small above the real line. Harmonic measure transfers a
hypothetical upper bound on a fixed measurable set to an inequality
for a positive convolution operator. Symmetry and the approximate
identity property contradict that inequality.
...
We first record a local form of Riesz's differentiation formula, including the
consequence of near equality that will be needed below.

\begin{lemma}\label{lem:riesz}
Suppose that \(F\) is holomorphic in a neighborhood of the square \(\{|\Re
z|,|\Im z|\le T\}\), is real on its real diameter, and satisfies
\[
 |F(u+iv)|\le e^{|v|}.
\]
...
We next apply this formula to interpolants with bounded nodal data.
...
\begin{proof}
We apply Tao \cite[Theorem 4.1]{Tao} to the interval \(I\) in
\eqref{eq:local-upper}, and use its conclusions on fixed compact subintervals
of \(\operatorname{int}I\). For the derivative estimates, choose such an
interval containing \(J\) in its interior; for \eqref{eq:quadrature}, choose
one containing the supports of the functions \(h_n\). Put
\[
 S_n=\sum_k\frac1{|P_n'(x_{k,n})|},\qquad
 \alpha_n=\frac1n\log S_n,\qquad
 U_n(z)=\frac1n\log\frac1{|P_n(z)|}.
\]
That theorem supplies the asserted bounds on \(\rho_n\), together with
...
\begin{equation}\label{eq:discrepancy}
 ...
 \qquad(A\ge1).
\end{equation}
Taking \(A=n^{1/10}\) and integrating by parts proves \eqref{eq:quadrature}.
The bounds are uniform for the stated class of \(h_n\).

For complex \(z\) with \(\Re z\) in a fixed compact subinterval of
\(\operatorname{int}I\) and \(\Im z=\eta\ge\log n/n\), the
...
\]
where \(D_0>0\) is fixed and sufficiently large. On the upper edge of the
rectangle
\[
...
\]
for all sufficiently large \(n\). Indeed, the allowance in \(\omega H\) has
order \(n^{1/2}\), whereas the density variation contributes \(O(n^{2/5})\),
and the remaining terms contribute \(O(\log n)\).
...
\]
For \(|u|\le L/2\) and \(0\le v\le H\), it yields
\begin{equation}\label{eq:local-growth}
 |q(x+u+iv)|\le\Lambda_n(1+n^{-10})e^{\omega|v|}.
\end{equation}
Here the error from the vertical sides is \(\exp(O(n e^{-c n^{1/5}}))\);
reflection supplies the estimate for negative \(v\).
...
\]
For convenience, the same expression may be defined at the first and last
nodes by using the exterior polynomial pieces. Set
\[
...
\]
The kernel \(K_\varepsilon\) is positive definite. For example, its representation
\[
 K_\varepsilon(t)=\frac1{\sqrt\pi}
       \int_0^\infty s^{-1/2}e^{-s\varepsilon^2}e^{-st^2}\,ds
\]
expresses it as a positive mixture of Gaussian kernels. Write
\[
...
\begin{proof}
Fix nested compact interior intervals and a function \(f\) as in
Lemma~\ref{lem:energy}. Let
\[
...
\end{equation}
with nonnegative summands for all sufficiently large \(n\). It follows that
there is a fixed \(D\) for which the nodes in the support of \(f\) satisfying
\begin{equation}\label{eq:large-jump}
 T_k\ge c_0\log n-D
\end{equation}
carry at least \(W/2\) of the weights in \eqref{eq:weights}. In particular,
there are at least \(\kappa n\) such nodes, with \(\kappa>0\) independent of
\(n\). Discarding the first and last nodes does not change this conclusion
...
These points cannot all concentrate in a small number of node gaps. To see
this, use the adjacent piece with the large derivative. Its values at the two
endpoints of that gap are both 1. Rolle's theorem and the second derivative
bound in \eqref{eq:derivative-bounds} imply that the gap has length at least
\(d/n\), for a fixed \(d>0\). If the gap leaves the fixed interior
neighborhood, its length is already bounded below by a positive constant, so
the same conclusion holds.

Any interval of length less than \(d/n\) contains at most two nodes having an
adjacent gap of length at least \(d/n\). Otherwise a middle node would have
both neighboring gaps shorter than \(d/n\). Thus every interval of length
\(O(n^{-1})\) contains only a bounded number of the nodes in
\eqref{eq:large-jump}. The displacement bound in \eqref{eq:high-points} gives
the same bound, with multiplicities, for the points \(y_k\). A greedy
selection supplies at least \(\kappa_1n\) points satisfying
\eqref{eq:high-points} and separated by \(d_1/n\), for fixed positive
\(\kappa_1,d_1\).
...
We now prove the almost-everywhere assertion. Again suppress the row index,
and put
\[
...
\begin{equation}\label{eq:transforms}
 ...
 g(z)=\sum_k\frac{\epsilon_k\nu_k}{z-x_k}
     =\frac1{SP(z)}.
\end{equation}
The last identity is the partial fraction formula for \(1/P\). Its expansion
at infinity also gives
...
\begin{proof}
The function \(t\mapsto(x+ih-t)^{-1}\) is analytic on the Bernstein ellipse of
parameter \(R=1+h/4\), where its modulus is \(O(h^{-1})\). Truncating its
Chebyshev expansion at degree \(n-2\) gives a polynomial \(p\) satisfying
\begin{equation}\label{eq:chebyshev-tail}
 \sup_{t\in[-1,1]}\left|\frac1{x+ih-t}-p(t)\right|
 \le C h^{-2}e^{-cnh}.
\end{equation}
Indeed, the Chebyshev coefficients are \(O(h^{-1}R^{-j})\), and their tail is
a geometric series. The signed measure in \eqref{eq:transforms} has total
variation one and annihilates \(p\), by \eqref{eq:moments}. Thus
\eqref{eq:chebyshev-tail} bounds \(|g(x+ih)|\). On the other hand,
\[
 \gamma_h(x)=\sum_k\frac{\nu_kh}{(x-x_k)^2+h^2}
 \ge\frac{h}{4+h^2}\qquad(x\in[-1,1]).
\]
This proves \eqref{eq:cancellation}.
\end{proof}
...
\]
By \eqref{eq:lebesgue-transform}, \(H^\eta\le H\). Direct differentiation
gives \(|\gamma_s'|\le\gamma_s/s\), and consequently
...
events true. We may take \(y\) away from the nodes. At this point,
\[
 |g(y)|\le K\gamma_h(x).
\]
Using \eqref{eq:lebesgue-transform}, \eqref{eq:common-low-set}, and
\eqref{eq:log-derivative}, we obtain
\begin{equation}\label{eq:local-comparison}
 H^\eta(x)\le e^{Rh/\eta}H^\eta(y)
 \le cK e^{Rh/\eta}(\log n)\gamma_h(x)
 \qquad(x\in E_n).
\end{equation}
...
\begin{equation}\label{eq:kernel-ratio}
 ...
 \quad(x\in E_n),\qquad
 A_n=\frac{2b}{\pi cK}e^{-Rh/\eta}\longrightarrow A>1.
\end{equation}
...
The proofs and manuscript were developed using GPT-6 Astra (OpenAI).
The author is solely responsible for the mathematical content of the paper.
