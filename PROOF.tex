...
\begin{equation}\label{eq:chebyshev-tail}
 ...
 \le C h^{-2}e^{-cnh}.
\end{equation}
The Chebyshev coefficients are \(O(h^{-1}R^{-j})\), so the tail is geometric.
The signed measure in \eqref{eq:transforms} has total variation one and
annihilates \(p\), by \eqref{eq:moments}. Thus
