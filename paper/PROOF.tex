...
The dependence of Tao's constant on \(I\) is already unavoidable when
the array varies with the interval: a smooth profile for \(|h|\), equal
to one on \(I\) and large outside, followed by polynomial approximation gives
\(\sup_I\lambda_n=(2/\pi)\log(n|I|)+O(1)\), hence
\(C_I\ge(2/\pi)\log(1/|I|)-O(1)\); see
the interval-length remark in Section~\ref{subsec:counter-amplitude}.
Theorem~\ref{thm:nonuniform} gives one
fixed array for which every finite constant fails on some interval.
...
Both main theorems and their set and interval-constant corollaries are
formalized in Lean; see the final section for the precise statements
and repository. The formal proof includes direct proofs of the local
potential estimates used from Tao.
...
The Lean statements \texttt{Erdos1132.theorem1} and
\texttt{Erdos1132.Counterexample.theorem2} have precisely the node-array
conditions and \(M>0\) as their hypotheses, respectively.
Both main theorems, the set corollary, and the interval-constant
corollary are formalized in Lean 4 with Mathlib. The formal proofs
follow the arguments above with the substitutions listed in the
repository README. The checks expand the Lagrange products and verify
that the proofs use only Lean's standard axioms \texttt{propext},
