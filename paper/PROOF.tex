...
The constant \(C\) may depend on the array and on \(x\).
...
constant at a fixed point. Baire category enters (i) at one point, to
...
\end{equation}
where \(C\) is fixed, and let \(\rho_n\) be the densities above.
...
we have \(|q(z)|\le\Lambda_n e^{\omega H}\): the allowance in
\(\omega H\) is of order \(n^{1/2}\), while the Lipschitz variation of
\(\rho_n\) contributes \(O(n^{2/5})\) and the remaining terms in
\eqref{eq:complex-interpolant} contribute \(O(\log n)\).
...
\(T\asymp_J n^{3/5}\).
Since \(\Lambda_n n^{-1/10}=o(1)\), \eqref{eq:riesz-derivative}
gives the first bound in \eqref{eq:derivative-bounds}; Cauchy's estimate
on a disk of radius comparable to \(n^{-1}\) gives the second.
Under \eqref{eq:large-derivative}, change the sign of \(q\) if necessary
so that \(F'(0)\ge1-C_1/\Lambda_n\).
...
\begin{corollary}
For every measurable \(E\subset[-1,1]\) of positive measure and every
\(c<2/\pi\), there are infinitely many \(n\) for which some \(x\in E\)
satisfies \(\lambda_n(x)>c\log n\).
\end{corollary}
\begin{proof}
Choose \(x\in E\cap(-1,1)\) satisfying Theorem~\ref{thm:main}(ii).
The inequality holds at this same point for infinitely many \(n\).
\end{proof}
For \(0<c<2/\pi\), the argument above proves this directly: any measurable
set supporting an eventual uniform bound \(\lambda_n\le c\log n\) has
measure zero. This is \texttt{Erdos1132.uniform\_low\_set\_null} in Lean.
...
Sections~\ref{sec:cauchy}--\ref{sec:ae}, has been formalized in Lean 4
with Mathlib using OpenAI Codex (GPT-6). The source and build instructions
