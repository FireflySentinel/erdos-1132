...
The corollary at the end of Section~\ref{sec:ae} gives, in every
...
eliminated.'' The following consequence of Theorem~\ref{thm:nonuniform}
establishes this for a single triangular array.
...
\]
for every nondegenerate interval \(I\subset(-1,1)\) and all sufficiently
...
nondegenerate subinterval \(I\). The resulting upper bound on \(I\)
...
All values reported by the script are floating-point evaluations.
\end{remark}
...
Both conclusions of Theorem~\ref{thm:main}, the set corollary, and the
complete Theorem~\ref{thm:nonuniform} with its interval-constant corollary
have been formalized in Lean 4 with Mathlib using OpenAI Codex (GPT-6).
The main statements are \texttt{Erdos1132.theorem1} and
\texttt{Erdos1132.Counterexample.}\allowbreak
\texttt{theorem2\_unshifted}; their hypotheses
are precisely the node-array conditions and \(M>0\), respectively.
The checks expand the Lagrange products and verify that the proofs use
only Lean's standard axioms \texttt{propext}, \texttt{Classical.choice},
and \texttt{Quot.sound}.
The source and build instructions are at
\url{https://github.com/FireflySentinel/erdos-1132}.

For Theorem~\ref{thm:main}(i), the formal proof uses circular contours
for the finite Riesz formula, obtains the required Lipschitz quadrature
estimate directly from the Poisson representation, and estimates the
high-value covers by bounded overlap. These replace the corresponding
auxiliary arguments above and yield the same conclusion. The local
potential, positive density, derivative, energy, and recurrence estimates
are all proved in Lean.
For Theorem~\ref{thm:nonuniform}, the formal proof includes the Cantor set,
the smooth sequence, the polynomial amplitudes, all analytic estimates of
Lemma~\ref{lem:amplitude}, and both conclusions for the same array.
It chooses smooth cutoffs from an open-set support function, with value
one on the same finite collections of gap middle halves, and uses a
fixed ratio-approximation error below \(1/2\), which suffices for both
conclusions.
