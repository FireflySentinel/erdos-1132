...
The complete Theorem~\ref{thm:nonuniform} is also formalized as
\texttt{Erdos1132.Counterexample.theorem2\_unshifted}, including the
construction of the Cantor set, the smooth sequence, the polynomial
amplitudes, all analytic estimates of Lemma~\ref{lem:amplitude}, and both
conclusions for the same array. Its only hypothesis is \(M>0\).
The formal proof chooses smooth cutoffs from an open-set support function,
with value one on the same finite collections of gap middle halves;
it uses a fixed ratio-approximation error below \(1/2\), which suffices
for both conclusions. The checks expand the Lagrange product and verify
that the proof uses only Lean's standard axioms \texttt{propext},
\texttt{Classical.choice}, and \texttt{Quot.sound}.
Theorem~\ref{thm:main}(i) remains outside the formalization.
