...
The two proofs use derivative-jump energies and positive Cauchy
transforms, respectively. Both are formalized in Lean 4.
\end{abstract}
...
Theorem~\ref{thm:main} answers the two questions in Erd\H{o}s
Problem~1132 \cite{Bloom}, with a point-dependent constant in part~(i).
An absolute constant cannot replace \(C(x)\): for every \(M>0\),
there is an array such that, at every \(x\in(-1,1)\),
\[
 \lambda_n(x)\le\frac2\pi\log n-M\qquad(n\ge n_0(x)).
\]
This construction is given in \cite[Theorem~1]{GuNonuniform}.
...
function for every array. Their quantitative estimate
\cite[Theorem~2.1]{ErdosVertesi1981} gives logarithmic growth outside
a set of arbitrarily small measure, with a positive coefficient
depending on that measure. In particular,
\[
 \limsup_{n\to\infty}\frac{\lambda_n(x)}{\log n}>0
 \quad\text{for almost every }x.
\]
Theorem~\ref{thm:main}(ii) gives the sharp coefficient \(2/\pi\).
...
The proofs of Theorem~\ref{thm:main}(i) and (ii) are independent.
We begin with the self-contained proof of part~(ii) in
Sections~\ref{sec:cauchy}--\ref{sec:ae}: cancellation in positive
Cauchy transforms and a harmonic-measure identity rule out a common
low set of positive measure.
For part~(i), Sections~\ref{sec:local}--\ref{sec:fixed} use Tao's local
estimates \cite[Theorem~4.1(ii)]{Tao}, together with Riesz's
differentiation formula, to turn large derivative jumps into nearby
points with a bounded additive loss.
A Riesz energy bound and a recurrence argument give a positive-measure
set of such points, and Baire's theorem gives density.
Section~\ref{sec:consequences} gives a positive-measure consequence.
...
The following proposition is Tao's local estimate
\cite[Theorem~4.1(ii), equations (4.6)--(4.7)]{Tao}; the normalization
and density are those of his equations (4.11) and (4.15).
We specialize his polynomial bound \(\sup_I\lambda_n\ll n^{O(1)}\)
to \(\sup_I\lambda_n\le n\).
...
Define the approximate local node density by
\[
...
\begin{enumerate}
\item[(i)] The densities are bounded above and below by positive constants
...
\[
 ...
 \qquad(x,x_0\in I').
\]
\item[(ii)] For \(x\in I'\) and \(\eta\ge\log n/n\),
...
\begin{lemma}[Local quadrature]\label{lem:quadrature}
Under the hypotheses of Proposition~\ref{prop:tao}, fix a compact
interval \(I'\Subset\operatorname{int}I\).
For globally Lipschitz functions \(\varphi_n\) supported in \(I'\),
with uniformly bounded supremum norms and Lipschitz constants,
...
\end{equation}
where \(C\) is fixed. This bound gives the hypothesis of
Proposition~\ref{prop:tao}; let \(\rho_n\) be its densities.
...
The maximum modulus principle gives \(|f(z)e^{-\Psi(z)}|\le1\).
...
\[
 \Delta_k=\lambda_n'(x_{k,n}+)-\lambda_n'(x_{k,n}-).
\]
...
\begin{equation}\label{eq:jump-average}
 \sum_k\frac{f(x_{k,n})^2}{n\rho_n(x_{k,n})}
       \frac{\Delta_k}{2\pi n\rho_n(x_{k,n})}
...
\[
 ...
 \widehat\Delta_k=\frac{\Delta_k}{2\pi n\rho_n(x_{k,n})},\qquad W=\int f^2>0.
\]
...
\begin{equation}\label{eq:jump-upper}
 0\le\widehat\Delta_k\le\Lambda_n+o(1)
\end{equation}
...
\begin{equation}\label{eq:weighted-deficit}
 \sum_kw_k(\Lambda_n+1-\widehat\Delta_k)=O(1),
\end{equation}
with nonnegative summands for all sufficiently large \(n\). On a node with
\(\widehat\Delta_k<c_0\log n-D\), the factor in \eqref{eq:weighted-deficit} is greater
...
\begin{equation}\label{eq:large-jump}
 \widehat\Delta_k\ge c_0\log n-D
\end{equation}
...
\(b>0\) and \(D_2>D_1+C_Ib\), and let \(A_n\) be the union of the
intervals of radius
\[
 r_n=\frac{b}{n\log n}
\]
...
\begin{equation}\label{eq:high-sets}
 \lambda_n(x)>c_0\log n-D_2\quad(x\in A_n),\qquad
...
\(n\) in an infinite set \(S\). Repeating the proof of
Theorem~\ref{thm:main}(ii) along \(S\) gives the same contradiction
\(|E|/A\ge|E|\) with \(A>1\).
\end{proof}
...
Theorem~\ref{thm:main} is \texttt{Erdos1132.theorem1} in
\path{Erdos1132/Theorems.lean}.
The two estimates quoted from Tao in Proposition~\ref{prop:tao} are
also proved directly in Lean, with no additional axioms.

\section*{Declaration of generative AI and AI-assisted technologies in the writing process}

GPT-6 Astra proposed the derivative-jump and positive Cauchy-transform
arguments, drafted the manuscript,
...
\bibitem{GuNonuniform}
Q.~Gu,
\emph{Nonuniform additive constants for Lebesgue functions},
preprint, 2026.
\url{https://github.com/FireflySentinel/erdos-1132/blob/main/paper/NONUNIFORM.pdf}.
