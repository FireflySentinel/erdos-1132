...
No absolute additive constant works for all arrays:
...
The proofs are formalized in Lean 4.
\end{abstract}
...
Theorem~\ref{thm:nonuniform} rules out two natural uniform
strengthenings of Theorem~\ref{thm:main}(i).

Write \(G_C(X)\) for the set in \eqref{eq:counter-good-set}
associated with the array \(X\). Consider the following statements,
where \(C\in\R\), \(X\) ranges over triangular arrays, and density
is in \((-1,1)\):
\[
\begin{aligned}
\text{(U1)}&\quad \exists C\ \forall X,\quad G_C(X)\ne\varnothing;\\
\text{(U2)}&\quad \forall X\ \exists C,\quad G_C(X)\text{ is dense};\\
\text{(U3)}&\quad \forall X,\quad
 \bigcup_{C\in\R}G_C(X)\text{ is dense}.
\end{aligned}
\]
Theorem~\ref{thm:nonuniform} disproves (U1) and (U2), while
Theorem~\ref{thm:main}(i) proves (U3).
...
For triangular arrays, Erd\H{o}s \cite[p.~68, equation (7)]{Erdos}
stated that the set
of points \(x\) satisfying
\[
 \limsup_{n\to\infty}\frac{\lambda_n(x)}{\log n}\ge\frac2\pi
\]
is everywhere dense, deducing this from an interval estimate whose
proof he described as unpublished. He asked whether the normalized
bound holds almost everywhere, and separately whether the logarithmic
bound holds infinitely often at a point with a bounded additive loss.
Erd\H{o}s and V\'ertesi \cite{ErdosVertesi} proved
...
Corollary~\ref{cor:no-uniform-interval} shows that, for arbitrary
triangular arrays, the additive constant in the interval lower bound
cannot in general be chosen independently of the interval;
see also \cite[Remark~1.12]{Tao}.
...
The first two assertions below are Tao's local estimates
\cite[Theorem~4.1(ii), equations (4.6)--(4.7)]{Tao}; the normalization
and density are those of his equations (4.11) and (4.15).

\begin{taoestimates}[Local estimates]\label{prop:tao}
...
\end{equation}
\item For globally Lipschitz functions \(\varphi_n\) supported in \(I'\), with
uniformly bounded supremum norms and Lipschitz constants,
\begin{equation}\label{eq:quadrature}
 \frac1n\sum_k \varphi_n(x_{k,n})
 =\int \varphi_n(x)\rho_n(x)\,dx+O(n^{-1/4}).
\end{equation}
\end{enumerate}
\end{taoestimates}

\begin{proof}[Proof of (iii)]
Let \(\mu_n=n^{-1}\sum_k\delta_{x_{k,n}}\) and
\(Q_h=h^{-1}\int_h^{2h}p_t\,dt\), where
\(p_t(y)=t/(\pi(y^2+t^2))\). This is a probability kernel with
\(\int_{|y|>\sqrt h}Q_h(y)\,dy=O(\sqrt h)\), so
\(\|Q_h*\varphi_n-\varphi_n\|_\infty=O(\sqrt h)\).
The Poisson representation gives
\[
 (Q_h*\mu_n)(x)=\frac{U_n(x+ih)-U_n(x+2ih)}{\pi h}
 =\rho_n(x)+O\!\left(\frac{\log n}{nh}+h^2\right)
 \qquad(x\in I').
\]
Integrating against \(\varphi_n\) and using Fubini therefore gives
an error \(O(\sqrt h+\log n/(nh)+h^2)\) in
\eqref{eq:quadrature}. Taking \(h=n^{-1/2}\) proves the claim.
\end{proof}
...
On the lower edge \(|q|\le\Lambda_n\); the Bernstein--Walsh bound
...
\[
 ...
 \mathcal J_k=\frac{J_k}{2\pi n\rho_n(x_{k,n})},\qquad W=\int f^2>0.
\]
By \eqref{eq:quadrature},
\begin{equation}\label{eq:weights}
 \sum_kw_k=W+O(n^{-1/4}),\qquad 0\le w_k\le C_f/n.
\end{equation}
...
\begin{equation}\label{eq:jump-upper}
 0\le\mathcal J_k\le\Lambda_n+o(1)
\end{equation}
...
\begin{equation}\label{eq:weighted-deficit}
 \sum_kw_k(\Lambda_n+1-\mathcal J_k)=O(1),
\end{equation}
with nonnegative summands for all sufficiently large \(n\). On a node with
\(\mathcal J_k<c_0\log n-D\), the factor in \eqref{eq:weighted-deficit} is greater
...
\[
 ...
 \sigma_k=\operatorname{sgn}P'(x_k).
\]
Define
\begin{equation}\label{eq:transforms}
 m(z)=\sum_k\frac{\nu_k}{z-x_k},\qquad
 g(z)=\sum_k\frac{\sigma_k\nu_k}{z-x_k}
     =\frac1{SP(z)}.
\end{equation}
...
probability measures: their masses at \(x_k\) are \((1\pm\sigma_k)\nu_k\).
...
For \(\Im w<0\), define
\[
 B(w)=\frac1\pi\int_{S_0}
       \frac{-\Im w}{(t-\Re w)^2+(\Im w)^2}\,dt.
\]
The function \(B\) is harmonic and lies between zero and one.
Since \(F\) maps the upper half-plane into the lower half-plane,
\(B\circ F\) is bounded and harmonic. Away from the poles of \(F\)
and the preimages of the endpoints of \(S_0\), its boundary value is
\(\mathbf1_{\{F(y)\in S_0\}}\). These exceptions form a finite set:
\(F\) is a nonconstant rational function. The Poisson representation
of the bounded harmonic function \(B\circ F\) now gives
\eqref{eq:harmonic-measure}.
\end{proof}
...
\end{equation}
The argument below also applies if this bound holds only for \(n\)
in an infinite set: each estimate uses one row, and the final limit
can be taken along that set.
Choose constants
\[
...
Let \(\mathcal K_n\) be convolution with the symmetric probability kernel
\[
...
\[
 H^\eta=\frac{2b}{\pi}(\log n)\mathcal K_n\gamma_h.
\]
Thus \eqref{eq:local-comparison} becomes
\begin{equation}\label{eq:kernel-ratio}
 \frac{\mathcal K_n\gamma_h(x)}{\gamma_h(x)}\le\frac1{A_n}
 \quad(x\in E_n),\qquad
...
Fix \(A>1\). Choose \(0<\rho<1/16\) sufficiently small, as specified
in Subsection~\ref{subsec:counter-integral}, and put
\[
...
For \eqref{eq:counter-u6}, at least \((m_k-1)/2\) gaps lie on one side
of \(x\) within its stage \(k-1\) parent interval. Use the first
\(q=\lfloor m_k/8\rfloor\) on that side. The far endpoint of the
\(j\)-th such gap is at distance at most \(4j\ell_{k-1}/m_k\) from
\(x\), and its length is at least \(\delta_k\ell_{k-1}/m_k\).
These gaps contribute at least
\[
...
Fix such an \(x\), and write \(s=r(x)\), \(\Theta=\log(B/s)\), \(a_0=4s\).
For \(t\in[0,2]\), let
\[
...
\(r(y)\in[s/2,3s/2]\). On this range \(p'(r(y))\le4p(s)/s\), by the derivative formula for \(p\) and \(\Theta\ge16\). The integral of the absolute difference quotient over this part is thus at most \(4p(s)\). On \(s/2\le|y-x|<4s\), we have \(r(y)\le5s\) and
\[
 p(5s)/p(s)=\left(\frac{\Theta}{\Theta-\log5}\right)^\alpha\le2.
\]
...
\[
 \mathcal H=\sum_{\nu\in\{-,+\}}\max\{\log(l_\nu/(4s)),0\}.
\]
The positive part is exactly \(p(s)\mathcal H\). For the negative part,
...
\[
 h=\log(l_\nu/(4s)),\qquad \Theta_0=\Theta-\log5.
\]
Then \(0\le h<\Theta_0\), since \(\Theta_0-h=\log(4B/(5l_\nu))>0\), and
\begin{equation}\label{eq:counter-u12}
\int_{4s}^{l_\nu}\frac{p(5t/4)}t\,dt
 =\int_0^h(\Theta_0-z)^{-\alpha}\,dz
 \le\frac{\Theta_0^{-\alpha}}{1-\alpha}\,h.
\end{equation}
The last inequality follows from
\((1-z)^{1-\alpha}\ge1-z\) for \(0\le z\le1\).

Since \(\alpha=\eta/2\), concavity of the power \(\alpha\),
\(\log5<2\), and \(\Theta\ge16\) give
\begin{equation}\label{eq:counter-u13}
\begin{split}
 \frac{1-\eta}{1-\alpha}\frac{\Theta_0^{-\alpha}}{p(s)}
 &=\frac{1-\eta}{1-\alpha}
       \left(\frac{\Theta}{\Theta-\log5}\right)^\alpha\\
 &\le\frac{1-\eta}{1-\alpha}\left(1+\frac\alpha7\right)
 \le1-\frac\eta4=1-\frac1{512}.
\end{split}
\end{equation}
Thus the negative far contribution is at most
\(4p(s)+(1-1/512)p(s)\mathcal H\). Including the near part gives
\(Lu(x)/u(x)\ge \mathcal H/512-28\ge \mathcal H/512-32\).
At least one of \(l_-,l_+\) is at least one, so
\(\mathcal H\ge\log(1/(4s))\). This proves \eqref{eq:counter-u9}.

We now fix \(\rho\) small enough that
\[
 \log(2B/\rho)\ge16,\qquad
 \frac1{512}\log\frac1{2\rho}-32>A+2.
\]
Every gap has length at most \(|U|\le2\rho\), so \(r(x)\le\rho/2\).
Thus \(\Theta\ge16\), and \eqref{eq:counter-u9} yields
...
Let \(\epsilon_j=1/j\), \(j\ge2\). Since \(p(t)\to0\) as
\(t\downarrow0\), choose \(0<d_j<1\) with
\(p(d_j)\le\epsilon_j/2\), and put
\[
 C_j=2\log(2/d_j).
\]
Since \(r\le D\), one has
...
\begin{equation}\label{eq:counter-u16}
 ...
 \qquad(x\in F).
\end{equation}
Here the far integral is at most \(C_j\).

Choose an integer \(k_j\ge j\) such that
\[
 \frac{k_j}{32j}-C_j\ge(A+1)\epsilon_j.
\]
...
\end{equation}
This estimate is not uniform in \(x\). Indeed, at a minimum point
\(\xi_j\) of \(v_j\) on \(I\), one has \(Lv_j(\xi_j)\le0\).
No fixed point can minimize \(v_j\) for arbitrarily large \(j\).
...
\]
After amplitude approximation with error at most one in this quotient,
Lemma~\ref{lem:amplitude} gives
\[
 \sup_{I_\ell}\lambda_n\le\frac2\pi\log(n\ell)+O(1)
\]
for sufficiently large \(n\), with an absolute \(O(1)\) term and a
starting index depending on \(\ell\). Thus the interval constant can be at least
...
\[
 ...
 <\frac2\pi\log n-M.
\]
This proves \eqref{eq:counter-eventual} and the failure of an absolute
additive constant. The passage from \eqref{eq:counter-u19} to this
bound is pointwise in \(x\), although \eqref{eq:counter-u26} is uniform
for each fixed amplitude. An interval supremum can occur at points
that vary with \(n\).
...
GPT-6 Astra proposed the derivative-jump and positive Cauchy-transform
arguments and the counterexample construction, drafted the manuscript,
and generated the Lean formalization.
