...
We also show that no absolute additive constant works for all arrays:
...
These counterexamples concern arbitrary triangular arrays; the
uniform-constant question for nested node sequences remains open.
...
Theorem~\ref{thm:main}(ii) gives the sharp coefficient in an
almost-everywhere logarithmic-order lower bound already established
by Erd\H{o}s and V\'ertesi \cite{ErdosVertesi1981}; we recall their
more precise measure estimate below.
...
As Tao notes in \cite[Remark~1.12]{Tao}, the original question leaves
the allowed dependence of the additive constant unspecified.
The following result separates two uniform interpretations from
...
The corollary at the end of Section~\ref{sec:ae} gives a different conclusion:
every fixed positive-measure set contains a point with
\(\lambda_n(x)>c\log n\) in every sufficiently large row, for any
\(0<c<2/\pi\). Erd\H{o}s--V\'ertesi's theorem controls all points outside
a small exceptional set with coefficient \(\eta(\varepsilon)\);
our corollary controls at least one point in each fixed set with a
coefficient arbitrarily close to \(2/\pi\). Neither conclusion
subsumes the other.
...
eliminated.'' Theorem~\ref{thm:nonuniform} proves this nonuniformity
for arbitrary triangular arrays, even if the common constant is
allowed to depend on the array. Indeed, if one constant \(C\) gave
Tao's interval bound on every nondegenerate \(I\), with the starting
index allowed to depend on \(I\), the same Baire argument would make
\(G_{C+1}\) dense, contradicting Theorem~\ref{thm:nonuniform}.
...
Theorem~\ref{thm:nonuniform} is proved in
Section~\ref{sec:counterexample}, independently of the preceding
lower-bound arguments. A compact set with many small gaps supports an
explicit continuous function whose logarithmic integral ratio is
large in every gap. Smooth positive approximations also make this
ratio large at each fixed point of the compact set. An amplitude
construction converts these functions into interpolation arrays.
...
We prove Theorem~\ref{thm:nonuniform}. Smooth on \(I=[-1,1]\)
...
Choose a nondecreasing smooth \(q_0:\mathbb R\to[0,1]\), equal to zero on \((-\infty,1]\) and one on \([2,\infty)\). On a gap \((b,c)\), set
\[
 \chi_\tau(x)=q_0((x-b)/\tau)q_0((c-x)/\tau),
\]
and put \(\chi_\tau=0\) on \(F\). Only finitely many gaps, those of length greater than \(2\tau\), contribute. Therefore \(\chi_\tau\) is smooth on \(I\); it vanishes for \(D\le\tau\), equals one for \(D\ge2\tau\), and increases pointwise to \(1_U\) as \(\tau\downarrow0\).
...
A_{b_N/8}(x)\ge N/16\qquad(x\in F).
\end{equation}
Indeed, \(b_k\) decreases strictly with \(k\). If \(k\le N\), the middle half of every stage \(k\) gap has distance at least \(b_k/4\ge2(b_N/8)\) from its endpoints, so \(\chi_{b_N/8}=1\) there. The argument proving \eqref{eq:counter-u6}, restricted to these middle halves, gives at least \(1/16\) from each of the first \(N\) stages.
...
Using this for \(|x-y|<d_j\), and \(u\le1\) on the remainder, gives, uniformly for \(x\in F\),
\begin{equation}\label{eq:counter-u16}
\int_U\frac{\chi_\tau(y)u(y)}{|x-y|}\,dy
 \le\frac{\epsilon_j}{2}A_\tau(x)+C_j.
\end{equation}
Here the far integral is at most
\(2\log(2/d_j)\le C_j\).

Now specify the increasing integers and decreasing cutoffs
\[
 k_j=\lceil32(A+1)\rceil+64j(2j)^{256}+j,\qquad
 \tau_j=b_{k_j}/8.
\]
By \eqref{eq:counter-u15},
\[
 \frac{\epsilon_j}{2}A_{\tau_j}(x)-C_j
...
\end{equation}
This function is strictly positive and smooth on a neighborhood of \(I\): only finitely many compact portions of gaps contribute, \(u\) is smooth and positive there, and the function equals \(\epsilon_j\) near their boundaries and near the endpoints of \(I\).

At \(x\in F\), \eqref{eq:counter-u16} gives \(Lv_j(x)\ge(A+1)v_j(x)\). At every fixed \(x\in U\), eventually \(v_j=u\) on a neighborhood of \(x\). Also \(v_j\to u\) in \(L^1(I)\), by bounded convergence. The integrals outside that neighborhood then converge, so
...
\left\|\frac{L\widetilde v_j}{\widetilde v_j}
                 -\frac{Lv_j}{v_j}\right\|_\infty\le1/j.
\end{equation}
...
\(L\widetilde v_j(x)/\widetilde v_j(x)\ge A+1-1/j\) eventually. Hence \eqref{eq:counter-u26} gives, for all sufficiently large \(n\),
\[
 \lambda_n(x)\le\frac2\pi\log n+3-\frac{A+1-1/j}{\pi}
 <\frac2\pi\log n-M.
\]
...
An independent Codex agent also reviewed the complete proof of
Theorem~\ref{thm:nonuniform}.
...
Selected lemmas of Section~\ref{sec:counterexample}, including the
conditional assembly step, are also formalized; their precise scope
and hypotheses are documented with the source.
Theorem~\ref{thm:main}(i) and the complete
Theorem~\ref{thm:nonuniform} remain outside the formalization.
